...
import { Spinner, CopyIcon, UploadIcon, CheckIcon, FocusIcon, PrintIcon, Logo, SettingsIcon, TrashIcon } from './Shared.js';
...
const PMCAgent = ({ language, onOpenSettings }) => {
    ...
    // Multi-file state
    const [previews, setPreviews] = useState([]); // Array of { id, type, data, mimeType, name }
    ...
    const handleAnalyze = async (standardOverride = null) => {
        if ((!textInput.trim() && previews.length === 0) || isLoading) return;
        ...
        try {
            const inputItems = [];
            // Add manual text if any
            if (textInput.trim()) {
                inputItems.push({ type: 'text', data: textInput });
            }
            // Add file previews
            previews.forEach(p => {
                inputItems.push({ 
            ...
            const hasImages = previews.some(p => p.type === 'image');
            const analysis = await apiService.analyzeSchedule(inputItems, hasImages, standardToUse, language, selectedProvider);
            setResult(analysis);
        } catch (error) {
    ...
    const processFiles = (files) => {
        ...
        Array.from(files).forEach(file => {
            ...
            } else if (fileName.endsWith('.xer') || fileName.endsWith('.csv') || fileName.endsWith('.txt') || fileName.endsWith('.xml')) {
                const reader = new FileReader();
                reader.onload = (e) => {
                    setPreviews(prev => [...prev, {
                        id: fileId,
    ...
    return React.createElement('div', { className: "max-w-7xl mx-auto pb-12 animate-fade-in printable-report relative" },
        React.createElement('div', { className: "print-only border-b border-black mb-4 pb-2" },
            React.createElement('div', { className: "flex justify-between items-center" },
                React.createElement('div', { className: "flex items-center gap-2" },
                    React.createElement(Logo, { className: "h-8 w-auto grayscale" }),
        ...
        (!result || isLoading && !result) && React.createElement('div', { className: "grid grid-cols-1 lg:grid-cols-12 gap-8 no-print mb-12" },
            ...
            React.createElement('div', { className: "lg:col-span-8 flex flex-col gap-6" },
                ...
                // Previews area
                previews.length > 0 && React.createElement('div', { className: "flex flex-wrap gap-4 p-4 bg-white dark:bg-card-gradient rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg" },
                    previews.map((p) => (
                        React.createElement('div', { key: p.id, className: "relative group w-24 h-24 bg-slate-100 dark:bg-white/5 rounded-lg overflow-hidden border dark:border-white/10" },
                            p.type === 'image' ? (
                                React.createElement('img', { src: p.data, alt: p.name, className: "w-full h-full object-cover" })
                            ) : (
                                React.createElement('div', { className: "w-full h-full flex flex-col items-center justify-center p-2 text-center" },
                                    React.createElement('svg', { className: "w-6 h-6 text-brand-primary mb-1", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor" }, React.createElement('path', { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" })),
                                    React.createElement('span', { className: "text-[10px] font-bold truncate w-full px-1" }, p.name)
                                )
                            ),
                            React.createElement('button', { 
                                ...
                                React.createElement('svg', { className: "w-3 h-3", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor" }, React.createElement('path', { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }))
                            )
                        )
                    ))
                ),
                ...
                React.createElement('div', { className: "bg-white dark:bg-card-gradient rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg overflow-hidden flex flex-col h-full" },
                    ...
                    errorMsg && React.createElement('div', { className: "px-5 py-2" },
                        React.createElement('div', { className: "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-200 p-3 rounded-lg text-sm border border-red-200 dark:border-red-800 flex items-center justify-between gap-2" },
                            ...
                            (errorMsg.includes('configure') || errorMsg.includes('Settings')) && React.createElement('button', {
                                onClick: onOpenSettings,
                                className: "bg-red-100 hover:bg-red-200 text-red-800 dark:bg-red-800 dark:text-red-100 px-3 py-1 rounded-md text-xs font-bold transition-colors"
                            }, "Settings")
                        )
                    ),
                    ...
                    React.createElement('div', { className: "p-4 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 flex justify-end" },
                        React.createElement('button', {
                            ...
                            disabled: isLoading || (!textInput.trim() && previews.length === 0),
        ...
        isLoading && React.createElement('div', { className: "fixed inset-0 bg-slate-900/50 dark:bg-black/70 z-50 flex items-center justify-center backdrop-blur-sm animate-fade-in" },
            React.createElement('div', { className: "bg-white dark:bg-card-gradient p-8 rounded-3xl shadow-2xl flex flex-col items-center border border-slate-100 dark:border-white/10 max-w-sm w-full mx-4 relative overflow-hidden" },
                ...
                React.createElement('h3', { className: "text-xl font-bold text-slate-900 dark:text-white text-center mb-2" }, 
                    result ? (language === Language.AR ? "جاري تحديث المعايير..." : "Updating Standards...") : (language === Language.AR ? "جاري تحليل الجدول..." : "Analyzing Schedule...")
                ),
                React.createElement('p', { className: "text-slate-500 dark:text-brand-text-light text-center text-sm" }, 
                     language === Language.AR ? "يقوم المساعد الذكي بمراجعة البيانات، يرجى الانتظار..." : "AI Assistant is reviewing the data, please wait..."
                )
            )
        ),
             ...
             React.createElement('div', { className: "flex flex-col sm:flex-row justify-between items-center mb-6 no-print gap-4" },
                 React.createElement('div', { className: "flex items-center gap-3 bg-white dark:bg-brand-light-dark p-2 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm" },
                    ...
                    React.createElement('select', {
                        ...
                        className: "bg-slate-100 dark:bg-black/20 text-slate-800 dark:text-white text-sm font-semibold rounded-lg p-2 border-none outline-none"
                    },
                        PLANNING_STANDARDS.map(std => 
                 ...
                 React.createElement('div', { className: "flex gap-4" },
                    React.createElement('button', {
                        onClick: () => { setResult(null); setTextInput(''); setPreviews([]); setErrorMsg(null); },
                     ...
                     React.createElement('button', {
                        ...
                        className: "flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded-lg shadow-md"
                    },
